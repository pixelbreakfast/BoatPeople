...
public class SpawnActor : MonoBehaviour {
	...
	public List<Actor> Spawn () {
		...
		for(int x = 0; x < width;x++) {
			...
			for(int z = 0; z < length;z++) {
				...
				if(Physics.Raycast(ray,out hit, raycastRange)) {
					...
					actors.Add (newNPC.GetComponent<Actor>());
		...
		for(int i = 0; i < players; i++ ) {
			...
			GameObject newPC = GameObject.Instantiate(Resources.Load("Prefabs/Playable Boat Person")) as GameObject;
			actors.Add (newPC.GetComponent<Actor>());
		}
